...
// GitHub enchaine PLUSIEURS redirections pour un fichier de release
// (/latest/download -> /download/<tag> -> objects.githubusercontent.com).
// httpUpdate ne les suit pas, donc on parcourt toute la chaine ici pour
// obtenir l'URL finale (celle qui renvoie le binaire en 200).
String resoudreRedirection(const char* base) {
  String url = base;
  for (int hop = 0; hop < 6; hop++) {
    ...
    if (code == 301 || code == 302 || code == 303 || code == 307 || code == 308) {
      ...
      http.end();
      if (loc.length() == 0) return url;
      url = loc;                 // on suit la redirection suivante
    } else {
      http.end();
      return url;                // plus de redirection : URL finale
    }
  }
  return url;
}
